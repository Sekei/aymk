...
public class JoinSuccessPresenter extends BasePresenter<IJoinSuccessView>{

    public JoinSuccessPresenter(App app) {
        super(app);
    }
 ...
 //用户详情
    public void CancelApplyMearchants(Map<String, String> parmer) {
        if (isViewAttached()) {
            getView().showProgress();
        }
        getAppComponent().getAPIService()
                .cancelApplyMearchants(parmer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<HttpResult<String>>() {
                    ...
                    public void onNext(HttpResult<String> userBeanHttpResult) {
                        ...
                        if (userBeanHttpResult != null) {
                            if (isViewAttached())
                                getView().onCancelApplyMearchants(userBeanHttpResult.getData());
                        }
                    }
                });
    }
