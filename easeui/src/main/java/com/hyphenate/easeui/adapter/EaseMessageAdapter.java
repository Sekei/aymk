...
import com.hyphenate.easeui.widget.presenter.EaseChatHealthFilePresenter;
...
import com.hyphenate.easeui.widget.presenter.EaseChatMedicalRecordPresenter;
import com.hyphenate.easeui.widget.presenter.EaseChatRowPresenter;
import com.hyphenate.easeui.widget.presenter.EaseChatTestDataPresenter;
...
public class EaseMessageAdapter extends BaseAdapter {
    ...
    private static final int MESSAGE_TYPE_SENT_TESTDATA = 14;
    private static final int MESSAGE_TYPE_RECV_TESTDATA = 15;  //添加测试数据
    private static final int MESSAGE_TYPE_SENT_HEALTH_FILE = 16;//健康档案
    private static final int MESSAGE_TYPE_RECV_HEALTH_FILE = 17;

    private static final int MESSAGE_TYPE_SENT_MEDICAL_RECORD = 18;
    private static final int MESSAGE_TYPE_RECV_MEDICAL_RECORD = 19;//电子病历
    ...
    public int getItemViewType(int position) {
        ...
        if (message.getType() == EMMessage.Type.TXT) {
            ...
            if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_TEST_DATA, false)) {
                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_TESTDATA : MESSAGE_TYPE_SENT_TESTDATA;
            }


            //添加健康档案
            if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_HEALTH_FILE, false)) {
                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_HEALTH_FILE : MESSAGE_TYPE_SENT_HEALTH_FILE;
            }
    ...
    protected EaseChatRowPresenter createChatRowPresenter(EMMessage message, int position) {
        ...
        switch (message.getType()) {
            case TXT:
                ...
                } else if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_TEST_DATA, false)) {

                    //测试数据
                    presenter = new EaseChatTestDataPresenter();
                } else if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_HEALTH_FILE, false)) {
                    //健康档案
                    presenter = new EaseChatHealthFilePresenter();
                } else  if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_MEDICAL_RECORD, false)) {
                    //电子病历
                    presenter = new EaseChatMedicalRecordPresenter();
                } else {
