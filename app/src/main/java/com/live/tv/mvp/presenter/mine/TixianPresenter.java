...
public class TixianPresenter extends BasePresenter<ITixianView>{
    ...
    public TixianPresenter(App app) {
        super(app);
    }
    ...
    public void getUserDetail(Map<String, String> parmer) {
      ...
      //提现
    public void applyWithdrawals(Map<String, String> parmer) {
        if (isViewAttached()) {
            getView().showProgress();
        }
        getAppComponent().getAPIService()
                .applyWithdrawals(parmer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<HttpResult<String>>() {
                    ...
                    public void onNext(HttpResult<String> userBeanHttpResult) {
                        ...
                        if (userBeanHttpResult != null) {
                            if (isViewAttached())
                                getView().onapplyWithdrawals(userBeanHttpResult.getData());
                        }
                    }
                });
    }
    ...
    //收益提现
    public void applyProfitWithdrawals(Map<String, String> parmer) {
        if (isViewAttached()) {
            getView().showProgress();
        }
        getAppComponent().getAPIService()
