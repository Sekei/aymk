...
public class AssessmentOrderPresenter extends BasePresenter<IAssessmentOrderView>{

    public AssessmentOrderPresenter(App app) {
        super(app);
    }
    ...
    //评价订单
    public void assessmentOrder(Map<String, String> parmer) {
        if (isViewAttached()) {
            getView().showProgress();
        }
        getAppComponent().getAPIService()
                .assessmentOrder(parmer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
