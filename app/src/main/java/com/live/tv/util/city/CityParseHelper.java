...
public class CityParseHelper {
    ...
    private final String cityScreening = "市,盟,区,位,州,县";
    private final String districtScreening = "县,区,市,旗,州,域";
    ...
    public void initData(Context context) {
        ...
        for (int p = 0; p < mProvinceBeanArrayList.size(); p++) {
            ...
            for (int j = 0; j < cityList.size(); j++) {
                ...
                String ends = cityList.get(j).getName().substring(cityList.get(j).getName().length()-1,cityList.get(j).getName().length());
                if(!cityScreening.contains(ends)){
                    cityList.get(j).setName(cityList.get(j).getName()+"市");
                }
                ...
                for (int k = 0; k < districtList.size(); k++) {
                    ...
                    String dends = districtList.get(k).getName().substring(districtList.get(k).getName().length()-1,districtList.get(k).getName().length());
                    if(!districtScreening.contains(dends)){
                        districtList.get(k).setName(districtList.get(k).getName()+"市");
                    }
